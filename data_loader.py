...
# 现在假设图片及标签形式为. filename1.jpg ,filename1.txt
# filename1.txt 内容为， x1, y1, x2, y2, x3, y3, x4, y4
...
class mytraindata(DATA.Dataset):
    ...
    def show_result(self, image, polys, tags):
        for poly, tag in zip(polys, tags):
            cv2.line(image, (poly[0][0],poly[0][1]), (poly[1][0], poly[1][1]),(0,255,0),1)
            cv2.line(image, (poly[0][0],poly[0][1]), (poly[3][0], poly[3][1]),(0,255,0),1)
            cv2.line(image, (poly[2][0], poly[2][1]), (poly[1][0], poly[1][1]), (0, 255, 0), 1)
            cv2.line(image, (poly[2][0], poly[2][1]), (poly[3][0], poly[3][1]), (0, 255, 0), 1)
        cv2.imshow('', image)
        cv2.waitKey()
    ...
    for i in range(len(scale_ratio)):
        ...
        for poly_idx, poly_tag in enumerate(zip(polys, tags)):
            ...
            for shrinked_poly in shrinked_polys:
                ...
                gt_map = cv2.fillPoly(gt_map, [np.array(shrinked_poly).astype(np.int32)], 1)
...
if __name__ == '__main__':
    img_path = r'D:\work\vivian\data\train\ch4_training_images'
    label_path = r'D:\work\vivian\data\train\ch4_training_localization_transcription_gt'
